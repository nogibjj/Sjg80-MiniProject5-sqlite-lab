...
def load(dataset="data/DataSet_withNames.csv"):
    ...
    c.execute("CREATE TABLE DB_Email_Names (id,gender,birthdate,maiden_name,lname,fname,address	city,state,zip,phone,email,cc_type,cc_number,cc_cvc,cc_expiredate)")
    #insert
    c.executemany("INSERT INTO DB_Email_Names VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?)", payload)
    conn.commit()
    conn.close()
